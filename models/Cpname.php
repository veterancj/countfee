<?php
class Cpname extends CActiveRecord{
	/**
	...
	static public function getCpnameID($name){
		if(empty($name)) return 0;
		$arr = Cpname::model() -> find("name = '$name'");
		return $arr -> ID;
	}
	static public function getCpnamelist(){
		$sql = 'select * from basecpnameinfo order by cdate desc';
	...
	static public function getPageList($pageSize = 10,$name='',$number=''){
	    ...
	    $count = Cpname::model()->count($criteria);
	    $allnum = $count/$pageSize;
	    $pages=new CPagination($count);
		$pages->pageSize=$pageSize;
		$pages->applyLimit($criteria);
		$infoobj = Cpname::model()->findAll($criteria);
		return (object)array(
			'count'=>$count,
			'allnum'=>$allnum,
			'pages'=>$pages,
