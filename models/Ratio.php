<?php
class Ratio extends CActiveRecord{
	/**
	...
	static public function getRatiolist(){
		$sql = 'select * from baseratioinfo order by cdate desc';
	...
	static public function getPageList($pageSize = 10,$name='',$number=''){
	    ...
	    $criteria->order = 'number ';
	  	$count = Ratio::model()->count($criteria);
	    $allnum = $count/$pageSize;
	    $pages=new CPagination($count);
		$pages->pageSize=$pageSize;
		$pages->applyLimit($criteria);
		$infoobj = Ratio::model()->findAll($criteria);
		return (object)array(
			'count'=>$count,
			'allnum'=>$allnum,
			'pages'=>$pages,
